...
module.exports = function (__opts) {
  ...
  var privateProperties = (opts.privateProperties && opts.privateProperties.filter(function (prop) {
  ...
  var _save = opts.save;
  opts.save = function (model, cb) {
    if (model.__old) {
      privateProperties.forEach(function (prop) {
        model[prop] = model[prop] || model.__old[prop];
      });
    }
    if (_save) {
      _save(model, cb);
    ...
    app.hook('model:afterSave:' + opts.name).last(function (model, cb) {
      ...
      app.redis.ZREM(sortedSets[key], model.id, function (err) {
        ...
        if (typeof model[key] === 'string') {
          ...
          //In redis sorted sets, values with the same score are returned lexigraphically
          // so we store strings with score 0 to take advantage of this.
          // Store the string as [value]___[id] so we sort by value but have the id for loading
          app.redis.ZADD(sortedSets[key], 0, model[key] + '___' + model.id, cb);
        }
        else {
  ...
  store.list = function (query, options, cb) {
    ...
    if (options.sort) {
      ...
      app.redis[cmd](sortedSets[options.sort], opts.offset, opts.end, function (err, results) {
        ...
        sortedIds = results.map(function (result) {
          if (result.match(/___/)) {
            return result.split('___')[1];
          }
          return result;
        });
        if (!--latch) onDone();
  ...
  store.load = function (id, options, cb) {
    ...
    function onLoaded (err, model) {
      if (err) return cb(err);
      if (model && !options.raw) {
        privateProperties.forEach(function (prop) {
          delete model[prop];
        });
      }
    ...
    if (entity) {
      ...
      if (keys[0] !== 'id') {
        app.redis.GET(indexes[keys[0]], function (err, id) {
          if (err) return cb(err);
          if (!id) return cb();
          return collection.load(id, options, onLoaded);
        });
      }
      else {
        return collection.load(id, options, onLoaded);
